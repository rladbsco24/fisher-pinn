...
method notes. Because RK4 has no Newton iteration or nonlinear solve residual,
the Newton-specific columns are removed instead of being replaced by artificial
RK4-only columns. The 1D PNG tables keep the DataFrame index column, while the
